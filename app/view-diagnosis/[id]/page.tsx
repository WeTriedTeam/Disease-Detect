...
export default function DiagnosisDetailPage() {
  ...
  useEffect(() => {
    async function fetchDiagnosisDetail() {
      try {
        const res = await fetch(`localhost:3001/records${id}`); // API link to retrieve data per diagnosis_id
  ...
  useEffect(() => {
    if (diagnosisDetail && diagnosisDetail.boxes) {
      const boxes = diagnosisDetail.boxes.map((line) => {
        const [classId, xCenter, yCenter, width, height] = line
          .split(" ")
          .map(Number);
      ...
      setBoundingBoxes(
        boxes.filter(
          (box) => box.xCenter !== undefined && box.yCenter !== undefined
        )
      );
    }
  }, [diagnosisDetail]);
      ...
      {diagnosisDetail && boundingBoxes.length > 0 ? (
        <>
          <div className="flex flex-row">
            <h1 className="text-lg font-semibold md:text-2xl basis-2/6">
              Diagnosis Detail No.{diagnosisDetail?.id}{" "}
            </h1>
            <div className="basis-3/6"></div>
            <Button className="basis-1/6 justify-self-end" onClick={toggleEdit}>
              {editable ? "Save" : "Edit"}
            </Button>
          </div>
            ...
            <div className="mt-8 mb-8 grid grid-cols-2 grid-rows-1 gap-11 divide-x">
              ...
              >
                <img
                  ...
                  src={`${diagnosisDetail.imgRef}${diagnosisDetail.filename}.jpg`}
                  alt="CT Scan Image"
                />
                    ...
                    <div className="grid w-full items-center gap-4">
                      <div className="flex flex-col space-y-1.5">
                        ...
                        <Input
                          readOnly
                          id="patientName"
                          value={`${diagnosisDetail.patientName}`}
                        />
                      </div>

                      <div className="flex flex-col space-y-1.5">
                        <Label htmlFor="dateOfDiagnosis">
                          Date of Dianosis
                        </Label>
                        <Input
                          readOnly
                          id="dateOfDiagnosis"
                          value={`${diagnosisDetail.date}`}
                        />
                      </div>
                      ...
                      <div className="flex flex-col space-y-1.5">
                        ...
                        <Input
                          id="diagnosisNote"
                          value={`${diagnosisDetail.diagnosisNote}`}
                        />
                      </div>
                              ...
                              <Card className="mt-2 mb-2" key={index}>
                                ...
                                <CardContent>
                                  ...
                                  <Input
                                    className="mb-2"
                                    value={`${
                                      (box.xCenter - box.width / 2) * 416
                                    }`}
                                  ></Input>
                                  <label htmlFor="">Y:</label>
                                  <Input
                                    value={`${
                                      (box.yCenter - box.height / 2) * 416
                                    }`}
                                  ></Input>
                                </CardContent>
                              </Card>
